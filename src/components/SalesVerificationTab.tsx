...
import { FileCheck, FileSpreadsheet, ArrowRight, Loader2, RefreshCw, AlertCircle, Download, AlertTriangle, CheckCircle2, ScanLine, Building2, FileStack, ChevronUp, ChevronDown } from 'lucide-react';
    ...
    <div className="space-y-4 mt-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="p-3 rounded-lg bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 text-center">
          <p className="text-xs text-blue-700 dark:text-blue-400">Само в дневник</p>
          <p className="text-xl font-bold text-blue-600">{summary.onlyInMainCount}</p>
        </div>
      ...
      {mismatched.length > 0 && (
        <div className="space-y-2">
          ...
          {mismatched.map((item, idx) => {
            const key = `mismatch-${idx}`;
            const isExpanded = expandedItems.has(key);
            return (
              <div key={key} className="border border-amber-200 dark:border-amber-800 rounded-lg overflow-hidden bg-amber-50/50 dark:bg-amber-950/10">
                <button
                  className="w-full flex items-center justify-between p-3 text-left hover:bg-amber-100/50 dark:hover:bg-amber-900/20"
                ...
                {isExpanded && (
                  <div className="border-t border-amber-200/50 p-3 bg-background/50 grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          {showOnlyInMain && (
            <div className="mt-2 space-y-1">
              {onlyMain.map((item, idx) => (
                <div key={idx} className="text-sm p-2 rounded bg-blue-50/50 dark:bg-blue-950/10 border border-blue-200/50 dark:border-blue-800/50">
      ...
      {/* All matched (collapsed summary) */}
      {matched.length > 0 && (
        <p className="text-sm text-emerald-600 dark:text-emerald-400">
          <CheckCircle2 className="h-4 w-4 inline mr-1" />
          {matched.length} документа съвпадат напълно
        </p>
      )}
    </div>
  );
